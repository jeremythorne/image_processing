...
impl RBriefPair {
    ...
    fn overlaps(&self) -> bool {
        (self.x1 - self.x2).abs() < 5 && (self.y1 - self.y2).abs() < 5
    }
...
impl Iterator for RBriefPairIter {
    type Item = RBriefPair;
    fn next(&mut self) -> Option<RBriefPair> {
        if !self.pair.valid() {
            ...
            loop {
                if self.pair.x2 < 13 {
                    self.pair.x2 += 1;
                } else if self.pair.y2 < 13 {
                    self.pair.y2 += 1;
                    self.pair.x2 = -13;
                } else if self.pair.x1 < 13 {
                    self.pair.x1 += 1;
                    self.pair.y2 = self.pair.y1;
                    self.pair.x2 = self.pair.x1;
                } else {
                    self.pair.y1 += 1;
                    self.pair.x1 = -13;
                    self.pair.y2 = self.pair.y1;
...
mod tests {
    ...
    #[test]
    fn test_rbrief_pairs() {
        let mut i = RBriefPair::new().iter();
        assert_eq!(i.next(), Some(RBriefPair { x1: -13, y1: -13, x2: -8, y2: -13}));
        assert_eq!(i.next(), Some(RBriefPair { x1: -13, y1: -13, x2: -7, y2: -13}));
        let mut i = RBriefPair { x1: 13, y1: 12, x2: 13, y2: 13 }.iter();
        let _p = i.next();
        assert_eq!(i.next(), Some(RBriefPair { x1: -13, y1: 13, x2: -8, y2: 13}));
        let mut i = RBriefPair { x1: 8, y1: 13, x2: 13, y2: 13 }.iter();
        let _p = i.next();
        assert_eq!(i.next(), None);
        // ORB 2012 makes this 205590, but I think they are wrong
        // they appear to do -13 to 13 non inclusive - which with a 5x5 sub window only extends
        // from -15 to 14 inclusive i.e. a 30x30 patch, not 31x31.
        assert_eq!(RBriefPair::new().iter().count(), 240856);
    }
}
